...
class Object3d(object):
    ...
    def loadParameters(self,label=-1):
        ...
        self.type = label['type'] # 'Car', 'Pedestrian', ...
        #self.cls_id = self.cls_type_to_id(self.type) --> get from yaml fil
        self.truncation = label['truncation'] # truncated pixel ratio [0..1]
        self.occlusion = label['occlusion'] # 0=visible, 1=partly occluded, 2=fully occluded, 3=unknown
        self.alpha =  label['alpha'] # object observation angle [-pi..pi]

        # extract 2d bounding box in 0-based coordinates
        self.xmin = label['box2d']['left'] # left
        self.ymin = label['box2d']['top'] # top
        self.xmax = label['box2d']['right'] # right
        self.ymax = label['box2d']['bottom'] # bottom
        self.box2d = np.array([self.xmin,self.ymin,self.xmax,self.ymax])
        
        # extract 3d bounding box information
        self.h = label['box3d']['h'] # box height
        self.w = label['box3d']['w'] # box width
        self.l = label['box3d']['l'] # box length (in meters)
        self.t = label['box3d']['t'] # location (x,y,z) in camera coord.
        self.dis_to_cam = np.linalg.norm(self.t)
        self.ry = label['ry'] # yaw angle (around Y-axis in camera coordinates) [-pi..pi]
